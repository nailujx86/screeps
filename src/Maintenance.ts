export function checkForRepair(structures: Structure[]) {
    structures.forEach(structure => {
        ...
        if (repairableAtControlLevel(structure.room, structure)) {
            if (structure instanceof StructureWall && structure.hits < 150000 || !(structure instanceof StructureWall) && structure.hits < (structure.hitsMax / 3)) {
            ...
            if ((structure.hitsMax > 0 && (structure.hits / structure.hitsMax) > 0.5 || structure instanceof StructureWall && structure.hits >= 150000) && needsRepairCurrently)
